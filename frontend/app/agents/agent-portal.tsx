...
import { ArrowLeft, Check, Clipboard, Fingerprint, KeyRound, LogOut, MemoryStick, Plus, RefreshCw, RotateCw, Save, ShieldCheck } from 'lucide-react';
          ...
          <>
            ...
            <section className="memory-workspace">
              ...
              <div className="memory-forms">
                <form onSubmit={saveStrategy}><div className="memory-form__label"><span>01</span><strong>STRATEGY VERSION</strong></div><label htmlFor="memory-asset"><span>ASSET</span><Input id="memory-asset" value={asset} onChange={(event) => setAsset(event.target.value.toUpperCase())} /></label><label htmlFor="memory-timeframe"><span>TIMEFRAME</span><Input id="memory-timeframe" value={timeframe} onChange={(event) => setTimeframe(event.target.value)} /></label><label htmlFor="memory-thesis"><span>THESIS</span><Textarea id="memory-thesis" value={thesis} onChange={(event) => setThesis(event.target.value)} /></label><Button disabled={busy}><Save /> Save new version</Button></form>
                <form onSubmit={saveImpression}><div className="memory-form__label"><span>02</span><strong>MARKET IMPRESSION</strong></div><label htmlFor="memory-impression"><span>STATEMENT</span><Textarea id="memory-impression" value={impression} onChange={(event) => setImpression(event.target.value)} /></label><label htmlFor="memory-confidence"><span>CONFIDENCE / 0–1</span><Input id="memory-confidence" type="number" min="0" max="1" step="0.01" value={confidence} onChange={(event) => setConfidence(event.target.value)} /></label><Button disabled={busy}><MemoryStick /> Store impression</Button></form>
                <form onSubmit={saveCheckpoint}><div className="memory-form__label"><span>03</span><strong>RECOVERY CHECKPOINT</strong></div><label htmlFor="checkpoint-json"><span>JSON STATE</span><Textarea id="checkpoint-json" className="portal-code-input" value={checkpointState} onChange={(event) => setCheckpointState(event.target.value)} /></label><Button disabled={busy}><Save /> Save checkpoint</Button></form>
              </div>
